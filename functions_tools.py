...
def calculate_center_of_mass (df):
    tot_mass = np.sum(df["Mass"])
    x = np.sum(df["X"]*df["Mass"])/tot_mass
    y = np.sum(df["Y"]*df["Mass"])/tot_mass
    z = np.sum(df["Z"]*df["Mass"])/tot_mass
...
def old_star_filter(pfilter,data):
    age = data.ds.current_time - data[pfilter.filtered_type, "particle_creation_time"]
    filter = age.in_units('Gyr') > 5
    return filter
...
def calculate_angular_momentum(ds, center,valor, Rvir, particle_type):
    sp5 = ds.sphere(center, (valor, "kpc"))
    am = sp5.quantities.angular_momentum_vector(use_gas=False,use_particles=True,particle_type= particle_type )
    angular_momentum = am
...
# class apply_derived_field:
#     def __init__(self, ds, center, mean_v, matriz_rotacion):
#         self.ds = ds
#         self.center = center
